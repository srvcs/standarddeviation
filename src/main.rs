use srvcs_standarddeviation::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-standarddeviation listening");
    let deps = Deps {
        populationstddev_url: cfg.populationstddev_url.clone(),
    };
